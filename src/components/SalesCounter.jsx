...
export default function SalesCounter({ sales, isPulsing, isVisible = true, mode = 'center', copy }) {
    // 숫자에 쉼표 포맷 추가
    const formattedSales = sales.toLocaleString();

    if (mode === 'center') {
        return (
            <div className={`relative z-20 flex flex-col items-center justify-center text-center pointer-events-none text-[#4D545A] ${isVisible ? 'sales-counter-appear' : 'sales-counter-hidden'}`}>
    ...
    return (
        <div className={`sales-counter-copy-group z-20 flex flex-col items-center justify-center gap-[4px] text-center text-[#ffffff] pointer-events-none ${isVisible ? 'sales-counter-appear' : 'sales-counter-hidden'}`}>
