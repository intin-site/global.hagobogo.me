...
## 2026-03-14 20:19 - origin 원격 제거
- 앞으로 `intin-site/global.hagobogo.me`만 업로드 대상으로 사용하기 위해 git 원격 설정에서 `origin`을 제거
- 원격 저장소 목록을 `intin` 단일 대상으로 정리해 다른 저장소로 잘못 push할 가능성을 차단

## 2026-03-14 20:35 - 서버 기준 판매 카운트 구조 기획서 작성
- 판매 카운트를 브라우저가 아니라 서버가 공식값으로 관리하고, 웹페이지는 서버 증가를 감지해 파란 점 애니메이션 뒤 숫자를 올리는 구조를 기획
- 관리자 재설정 시 새 값이 공식 기준값이 되고 이후 다시 1씩 누적되는 흐름을 문서로 정리
- Apps Script 환경에서는 WebSocket보다 `주기적 폴링`이 현실적이라는 판단과, 큰 차이값 발생 시 애니메이션 제한 정책도 함께 제안

## 2026-03-14 20:40 - 시간 규칙 기반 자동 증가 조건 반영
- 서버 기준 판매 카운트 기획서를 `시간 규칙 기반 자동 증가` 방식으로 구체화
- Admin의 `제품 판매 빈도 설정 (Dot_blue 발생 빈도)` 값을 자동 증가 기준으로 사용하는 규칙과 `60초~100초` 폴링 기준을 문서에 반영
- Apps Script 특성상 조회 시점 재계산과 시간 트리거를 함께 쓰는 방식을 권장안으로 정리

## 2026-03-14 20:56 - 서버 기준 자동 증가 카운터 구조 구현
- Apps Script 판매 카운트 구조를 `누적 증가분 저장` 방식에서 `현재 공식값 + 마지막 자동 증가 시각` 방식으로 전환
- `Dot_blue 발생 빈도`의 최소/최대 평균값을 시간당 자동 증가량으로 계산하고, 공개 조회 시점마다 최신 누적값을 반영하도록 수정
- 대시보드는 `60초~100초` 랜덤 폴링으로 서버 값을 다시 읽고, 서버 증가를 감지했을 때만 파란 점을 한 개씩 발사해 애니메이션 뒤 숫자를 올리도록 변경
- 차이값이 큰 경우에는 대표 애니메이션만 재생하고 숫자는 최종 서버값으로 따라가게 정리
- `npm run lint`, `npm run build`를 다시 실행해 구조 전환 후에도 정적 검사와 빌드가 통과하는 것을 확인

## 2026-03-14 21:09 - 자동 증가 카운트 초기 시각 저장 버그 수정
- `SITE_LAST_SALES_TICK_AT` 값이 비어 있을 때 조회마다 현재 시각으로 다시 계산되어 자동 증가가 영원히 시작되지 않던 문제를 수정
- 마지막 자동 증가 시각이 비어 있거나 잘못된 경우, 첫 조회 시 서버에 초기 기준 시각을 실제로 저장하도록 Apps Script 로직을 보정
- 수정 후 `npm run lint`, `npm run build`를 다시 실행해 정적 검사와 빌드가 계속 통과하는 것을 확인
